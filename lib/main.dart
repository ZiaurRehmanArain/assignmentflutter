import 'package:flutter/material.dart';

void main() {
  runApp(const MyApp());
}

class MyApp extends StatefulWidget {
  const MyApp({ Key? key }) : super(key: key);

  @override
  State<MyApp> createState() => _MyAppState();
}

class _MyAppState extends State<MyApp> {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      home: Scaffold(
      
        appBar: AppBar(title: Text("profile"),
        leading: Icon(Icons.menu),
        actions: [Icon(Icons.notification_add),SizedBox(width: 20,)],
        
        ),
        backgroundColor: Colors.white,
      
        body:SingleChildScrollView(child: 
         Column(
        
          children: [
            Divider(color: Colors.transparent,),
            Container(child: CircleAvatar(
              backgroundImage:NetworkImage("data:image/png;base64,"),
            radius: 60 ,backgroundColor: Colors.black,
            
            ),
            decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(100)),
            border: Border.all(
              color: Colors.black,
              width: 2
            )

            )),
            
            SizedBox(height: 20,),
            Container(child: Center(child: Text("Zia.ur.Rehman \nandroid course ",style: TextStyle(fontSize: 30,fontWeight: FontWeight.w600),)),),
          SizedBox(height: 20,),
          Container(
            
            width: 400,
            height: 30,
            
            decoration: BoxDecoration(
              
            boxShadow: [
              BoxShadow(
                color:Colors.red,
                spreadRadius: 2,
                blurRadius: 10
              )
            ],
            
            color: Colors.green,

              border:Border.all(width: 2,color: Colors.blue)
              
            ),
            child: 
          
           Row(children: [
              
              // Divider(color: Colors.black,),
              SizedBox(width: 110,),
            Container( child:Center(child: Icon(Icons.whatsapp)),),
            SizedBox(width: 30,),
            
             Container(child: Center(child: Text("whatsapp number")),),
            ],)),
            SizedBox(height: 20,),
             Container(
            
            width: 400,
            height: 30,
            
            decoration: BoxDecoration(
              
            boxShadow: [
              BoxShadow(
                color:Colors.red,
                spreadRadius: 2,
                blurRadius: 10
              )
            ],
            
            color: Colors.blueAccent,

              border:Border.all(width: 2,color: Colors.blue)
              
            ),
            child: 
          
           Row(children: [
              
              // Divider(color: Colors.black,),
              SizedBox(width: 110,),
            Container( child:Center(child: Icon(Icons.facebook)),),
            SizedBox(width: 30,),
            
             Container(child: Center(child: Text("facebook")),),
            ],)),
            SizedBox(height: 20,),
             Container(
            
            width: 400,
            height: 30,
            
            decoration: BoxDecoration(
              
            boxShadow: [
              BoxShadow(
                color:Colors.red,
                spreadRadius: 2,
                blurRadius: 10
              )
            ],
            
            color: Colors.blueGrey,

              border:Border.all(width: 2,color: Colors.blue)
              
            ),
            child: 
          
           Row(children: [
              
              // Divider(color: Colors.black,),
              SizedBox(width: 110,),
            Container( child:Center(child: Icon(Icons.message_rounded)),),
            SizedBox(width: 30,),
            
             Container(child: Center(child: Text("Messager")),),
            ],)), SizedBox(height: 20,),
             Container(
            
            width: 400,
            height: 30,
            
            decoration: BoxDecoration(
              
            boxShadow: [
              BoxShadow(
                color:Colors.red,
                spreadRadius: 2,
                blurRadius: 10
              )
            ],
            
            color: Colors.white30,

              border:Border.all(width: 2,color: Colors.blue)
              
            ),
            child: 
          
           Row(children: [
              
              // Divider(color: Colors.black,),
              SizedBox(width: 110,),
            Container( child:Center(child: Icon(Icons.report_gmailerrorred)),),
            SizedBox(width: 30,),
            
             Container(child: Center(child: Text("gmail")),),
            ],)), SizedBox(height: 20,),
             Container(
            
            width: 400,
            height: 30,
            
            decoration: BoxDecoration(
              
            boxShadow: [
              BoxShadow(
                color:Colors.red,
                spreadRadius: 2,
                blurRadius: 10
              )
            ],
            
            color: Colors.white70,

              border:Border.all(width: 2,color: Colors.blue)
              
            ),
            child: 
          
           Row(children: [
              
              // Divider(color: Colors.black,),
              SizedBox(width: 110,),
            Container( child:Center(child: Icon(Icons.email)),),
            SizedBox(width: 30,),
            
             Container(child: Center(child: Text("Email")),),
            ],)), SizedBox(height: 20,),
             Container(
            
            width: 400,
            height: 30,
            
            decoration: BoxDecoration(
              
            boxShadow: [
              BoxShadow(
                color:Colors.pink,
                spreadRadius: 2,
                blurRadius: 10
              )
            ],
            
            color: Colors.blueAccent,

              border:Border.all(width: 2,color: Colors.blue)
              
            ),
            child: 
          
           Row(children: [
              
              // Divider(color: Colors.black,),
              SizedBox(width: 110,),
            Container( child:Center(child: Icon(Icons.telegram)),),
            SizedBox(width: 30,),
            
             Container(child: Center(child: Text("Telegram")),),
            ],))
          ],
        ),
           )     ),
    );
  }
}